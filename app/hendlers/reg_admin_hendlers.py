...
async def create_admin(message: Message, state: FSMContext):
    ...
    await rq.change_list_steps(tg_id=message.from_user.id, parametrs=['name_reply', 'reg.Reg_admin.name'], my_list='registration_back', action='append')
    await message.answer(text='Введіть ваше ім\'я', reply_markup= menu)




@admin_rt.message(reg.Reg_admin.name)
async def create_admin_name(message: Message, state: FSMContext):
    ...
    await state.set_state(reg.Reg_admin.phone_number)
    menu = kb.menu_to_keyboard['phone_number_reply']()
    await rq.change_list_steps(tg_id=message.from_user.id, parametrs=['phone_number_reply', 'reg.Reg_admin.phone_number'], my_list='registration_back', action='append')
    await message.answer(text='Введіть ваш номер телефону', reply_markup= menu)


@admin_rt.message(reg.Reg_admin.phone_number)
async def create_admin_phone_number(message: Message, state: FSMContext):
    ...
    await state.set_state(reg.Reg_admin.email)
    menu = kb.menu_to_keyboard['email_reply']()
    await rq.change_list_steps(tg_id=message.from_user.id, parametrs=['email_reply', 'reg.Reg_admin.email'], my_list='registration_back', action='append')
    await message.answer(text='Введіть вашу електронну пошту', reply_markup= menu)


@admin_rt.message(reg.Reg_admin.email)
async def create_admin_email(message: Message, state: FSMContext):
    ...
    menu = kb.menu_to_keyboard['registration_admin_end']()
...
@admin_rt.callback_query(F.data == 'change_admin')
async def change_data(callback: CallbackQuery, state: FSMContext):
    await callback.answer(text='Оберіть, що хочете змінити 👇')
    menu = kb.menu_to_keyboard['change_data_admin']()
    await rq.change_list_steps(tg_id=callback.from_user.id, parametrs=['change_data_admin'], my_list='callback_back', action='append')
    await callback.message.edit_reply_markup(reply_markup= menu)
    


@admin_rt.callback_query(F.data == 'confirm_admin')
async def confirm_registration(callback: CallbackQuery, state: FSMContext):

    data = await state.get_data()
    data_in_db = await rq.get_registration_data(callback.from_user.id)
    ...
    menu = kb.menu_to_keyboard['admin_menu'](await rq.admin_is_created()) if await rq.get_status(callback.from_user.id) == 'user' else kb.menu_to_keyboard['menu'](await rq.get_status(callback.from_user.id))
    
    
    await callback.message.answer(
        text='Дані збережено✅', 
        reply_markup= menu)
    await rq.change_list_steps(tg_id=callback.from_user.id, parametrs=None, my_list='callback_back', action='clear')
    await state.clear()


# Збереження ім'я
@admin_rt.callback_query(F.data == 'change_name_admin')
async def save_name(callback: CallbackQuery, state: FSMContext):
    ...
    await rq.change_list_steps(tg_id=callback.message.from_user.id, parametrs=['registration_admin_end'], my_list='change_data', action='append')
    await state.set_state(reg.Reg_admin.change_name)
   

@admin_rt.message(reg.Reg_admin.change_name)
...
@admin_rt.callback_query(F.data == 'change_phone_number_admin')
async def save_phone_number(callback: CallbackQuery, state: FSMContext):
    ...
    await rq.change_list_steps(tg_id=callback.message.from_user.id, parametrs=['registration_admin_end'], my_list='change_data', action='append')
    await state.set_state(reg.Reg_admin.change_phone_number)

@admin_rt.message(reg.Reg_admin.change_phone_number)
...
@admin_rt.callback_query(F.data == 'change_email_admin')
async def save_email(callback: CallbackQuery, state: FSMContext):
    ...
    await rq.change_list_steps(tg_id=callback.message.from_user.id, parametrs=['registration_admin_end'], my_list='change_data', action='append')
    await state.set_state(reg.Reg_admin.change_email)

@admin_rt.message(reg.Reg_admin.change_email)
